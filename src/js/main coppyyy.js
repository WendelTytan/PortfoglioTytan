...
async function loadTexts() {
  try {
    console.log("Carregando Texts...");
    
    await i18next.loadNamespaces("Texts");
  ...
  try {
    ...
    if (!Array.isArray(formations)) {
      ...
      return;
        ...
        <h2 class="text-3xl md:text-4xl font-bold text-white mb-12 text-center">
          ${etc.sobre1} <span class="text-[#9b87f5]">${etc.mim1}</span>
        </h2>
        <div id="formations-container" class="space-y-8"></div>
      `;

      //${i18next.t("formations:title")} //um bom jeito de pegar o array ao inves daquele emb rolio

      const formationsContainer = document.getElementById("formations-container");

      
    // Adiciona cada formação sem repetir o cabeçalho
