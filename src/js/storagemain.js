...
async function loadTexts() {
  try {
    ...
    //função salvar no local
    async function getEtcData() {
      const storedLang = localStorage.getItem("selectedLang") || i18next.language;
      const currentLang = i18next.language;

      // Se o idioma mudou, limpar cache e recarregar os dados
      if (storedLang !== currentLang) {
          localStorage.removeItem("etcData");
          localStorage.setItem("selectedLang", currentLang);
      }

      const localData = localStorage.getItem("etcData");

      if (localData) {
          return JSON.parse(localData);
      } else {
        i18next.loadNamespaces("etc");
        const etcData = i18next.t("etc:texto", { returnObjects: true })[0];
  ...
  try {
    ...
    async function getHeroData() {
      ...
      if (localData) {
          ...
          await i18next.loadNamespaces("hero"); // Garante que os dados estão carregados
          const heroData = i18next.t("hero:hero", { returnObjects: true });
    ...
    heroContainer.innerHTML = hero
        .map(
            ...
            <div class="flex justify-center space-x-6">
              <a href="${her.lingit}" target="_blank" rel="noopener noreferrer" class="transition-colors escuro-icons claro-icons">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
              </a>
              <a href="${her.linln}" target="_blank" rel="noopener noreferrer" class="transition-colors escuro-icons claro-icons">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
              </a>
              <a href="${her.lininsta}" class="transition-colors escuro-icons claro-icons">
  ...
  try {
    ...
    async function getAboutData() {
      ...
      if (localData) {
        ...
        i18next.loadNamespaces("about");
        const aboutData = i18next.t("about:about", { returnObjects: true });
    ...
    aboutSection.innerHTML = `
      <div class="container mx-auto px-6">
        ...
        ${etc.sobre_1} <span class="escuro-high claro-high">${etc.sobre_2}</span>
        </h2>
...
async function loadFormations() {
  try {
    ...
    async function getFormationsData() {
      ...
      // Se o idioma mudou, limpar cache e recarregar os dados
      if (storedLang !== currentLang) {
          localStorage.removeItem("formationsData");
          localStorage.setItem("selectedLang", currentLang);
      }

      const localData = localStorage.getItem("formationsData");

      if (localData) {
          return JSON.parse(localData);
      } else {
        i18next.loadNamespaces("formations");
        const formationsData = i18next.t("formations:formations", { returnObjects: true });

          localStorage.setItem("formationsData", JSON.stringify(formationsData));
    ...
    const formations = await getFormationsData()
    ...
    formationsSection.innerHTML = `
        <div class="container mx-auto px-6">
          <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light claro-light">
            ${etc.form_1} <span class="escuro-high claro-high">${etc.form_2}</span>
          </h2>
          <div class="space-y-8">
          </div>
        </div>
      ...
      .map(
        ...
        <div class="space-y-8">
          <div class="p-6 rounded-lg escuro-fundo escuro-glow claro-fundo claro-glow">
            <h3 class="text-xl font-semibold escuro-high claro-high">${form.nome}</h3>
            <p class="escuro-suave claro-suave">${form.instituicao} • ${form.inicio} - ${form.fim}</p>
            <p class="mt-4 escuro-light claro-light">${form.descricao}</p>
          </div>
        </div>
    `
      )
...
async function loadExperiences() {
  try {
    ...
    async function getExperienceData() {
      ...
      const localData = localStorage.getItem("experienceData");

      if (localData) {
          return JSON.parse(localData);
      } else {
        i18next.loadNamespaces("experience");
        const experienceData = i18next.t("experience:experience", { returnObjects: true });
    ...
    experienceSection.innerHTML = `
        <div class="container mx-auto px-6">
          <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light claro-light">
            ${etc.exp_1} <span class="escuro-high claro-high">${etc.exp_2}</span>
          </h2>
          <div class="space-y-8">
          </div>
        </div>
    ...
    experienceContainer.innerHTML = experience
      .map(
        (exp) => `
      <div class="p-6 rounded-lg escuro-fundo escuro-glow claro-fundo claro-glow">
        <h3 class="text-xl font-semibold escuro-high claro-high">${exp.funcao}</h3>
        <p class="escuro-suave claro-suave">${exp.empresa} • ${exp.inicio} - ${exp.fim}</p>
        <p class="mt-4 escuro-light claro-light">${exp.descricao}</p>
      </div>
    `
      )
      .join("");
  ...
  try {
    ...
    async function getCoursesData() {
      ...
      if (localData) {
        ...
        i18next.loadNamespaces("courses");
        const coursesData = i18next.t("courses:courses", { returnObjects: true });
    ...
    coursesSection.innerHTML = `
        <div class="container mx-auto px-6">
          <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light claro-light">
            ${etc.cur_1} <span class="escuro-high claro-high">${etc.cur_2}</span>
          </h2>
          <div class="grid">
    ...
    coursesContainer.innerHTML = courses
      .map(
        (cor) => { //mix-blend-difference era para ser bom
          ...
          return `
            <div class="certificate-${cor.tipo} p-6 rounded-lg border escuro-fundo claro-fundo">
              <h3 class="text-xl font-semibold escuro-high claro-high">${cor.nome}</h3>
              <p class="escuro-suave claro-suave">${cor.instituicao} • ${cor.ano}</p>
              <p class="mt-2 escuro-light claro-light">${cor.descricao}</p>
              <p class="text-sm mt-2 escuro-suave claro-suave">${cor.horasdesc} ${cor.horas}h</p>
      ...
      //função salvar no local
      async function getResumData() {
        const storedLang = localStorage.getItem("selectedLang") || i18next.language;
        const currentLang = i18next.language;

        // Se o idioma mudou, limpar cache e recarregar os dados
        if (storedLang !== currentLang) {
            localStorage.removeItem("resumData");
            localStorage.setItem("selectedLang", currentLang);
        }

        const localData = localStorage.getItem("resumData");

        if (localData) {
            return JSON.parse(localData);
        } else {
          i18next.loadNamespaces("etc");
          const resumData = i18next.t("etc:resum", { returnObjects: true });

            localStorage.setItem("resumData", JSON.stringify(resumData));
            return resumData;
        }
      }

      const resum = await getResumData();
      ...
      const resumHtml = resum
        .map((res) => {
          const typeSummaryHtml = Object.entries(courseTypeCount)
            .map(([tipo, count]) => 
              `<p class="text-sm">
                <strong class="escuro-high claro-high">${tipo}:</strong> 
                <span class="escuro-contraste claro-contraste">${count}</span> ${res.res_cur}
              </p>`
            )
            .join('');
          ...
          return `
            <div class="resum_resum p-4 rounded-lg shadow-md escuro-fundo claro-fundo">
              <h3 class="text-xl font-bold mb-5 text-center escuro-high claro-high">${res.resumo}</h3>
      ...
      async function getCompData() {
        ...
        const localData = localStorage.getItem("compData");

        if (localData) {
            return JSON.parse(localData);
        } else {
          i18next.loadNamespaces("etc");
          const compData = i18next.t("etc:comp", { returnObjects: true });
        ...
        (cont) => {
          ...
          return `
            <div class="cont_comp mt-8 text-center">
              ...
              <p class="text-lg mb-2 font-bold escuro-high claro-high">${cont.contagem}</p>
...
async function loadProjects() {
  try {
    ...
    projectsSection.innerHTML = `
        <div class="container mx-auto px-6">
          <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light claro-light">
            ${etc.projetos_1} <span class="escuro-high claro-high">${etc.projetos_2}</span>
          </h2>
...
//adicionei uma var para linkar a site do projeto, não esquecer de fazer isso no ouprojetos
    projectsContainer.innerHTML = projects
      .map(
        (project) => `
        <a href="${project.siteprojeto}" class="block">
          <div class="rounded-lg overflow-hidden escuro-fundo escuro-glow claro-fundo claro-glow">
            ...
            <div class="p-6">
              <h3 class="text-xl font-semibold escuro-high claro-high">${project.titulo}</h3>
              <p class="mt-2 escuro-light claro-light">${project.descricao}</p>
...
async function loadMore() {
  try {
    ...
    moreSection.innerHTML = `
      <div class="container mx-auto px-6">
        <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light claro-light">
          ${etc.mais_1} <span class="escuro-high claro-high">${etc.mais_2}</span>
        </h2>
            ...
            .map(
              (morecomp) =>
                mor.links && mor.links[morecomp] // Verifica se há um link associado ao hobby
                  ? `<li><a href="${mor.links[morecomp]}" target="_blank" rel="noopener noreferrer" class="transition-colors hover:underline escuro-light claro-light">${morecomp}</a></li>`
                  : `<li>${morecomp}</li>`
            )
            .join("")}
        </ul>
      ...
      <div class="grid md:grid-cols-2 gap-8 mx-auto">
        <div class="p-6 rounded-lg escuro-fundo claro-fundo">
          <div class="flex items-center mb-4">
            ...
            <h3 class="text-xl font-semibold escuro-high claro-high">${mor.mais_1}</h3>
          </div>
            ${moreHtml}
          </div>
          <div class="p-6 rounded-lg escuro-fundo claro-fundo">
            <div class="flex items-center mb-4">
              ...
              <h3 class="text-xl font-semibold escuro-high claro-high">${mor.mais_2}</h3>
            </div>
            <div class="space-y-4">
              <iframe src="${mor.linspotify}" width="100%" height="152" frameborder="0" allowtransparency="true" allow="encrypted-media" class="rounded-lg escuro-light claro-light"></iframe>
              <a href="${mor.linsoundcloud}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center hover:underline escuro-light claro-light">
    ...
    tabletopSection.innerHTML = `
        <div class="container mx-auto px-6">
          <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light-inv claro-light-inv">
            ${etc.ouprojetos_1} <span class="escuro-high-inv claro-high-inv">${etc.ouprojetos_2}</span>
          </h2>
      ...
      .map( //bg-[#1A1F2C]
        ...
        <a href="${project.siteprojeto}" class="block">
          <div class="project-card rounded-lg overflow-hidden escuro-fundo claro-fundo">
            ...
            <div class="p-6">
              <h3 class="text-xl font-semibold  escuro-high-inv claro-high-inv">${project.nome}</h3>
              <p class="mt-2 escuro-light-inv claro-light-inv">${project.descricao}</p>
              <div class="mt-4 flex gap-2">
...
async function loadContact() {
  try {
    ...
    contactSection.innerHTML = `
        <div class="container mx-auto px-6">
          <h2 class="text-3xl md:text-4xl font-bold mb-12 text-center escuro-light claro-light">
            ${etc.contato_1} <span class="escuro-high claro-high">${etc.contato_2}</span>
          </h2>
    ...
    contactContainer.innerHTML = contact
      .map(
        ...
        <div class="flex justify-center space-x-8">
          <a href="${cont.lingit}" target="_blank" rel="noopener noreferrer" class="transition-colors escuro-icons claro-icons">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
            <span class="sr-only">${cont.git}</span>
          </a>
          <a href="${cont.linln}" target="_blank" rel="noopener noreferrer" class="transition-colors escuro-icons claro-icons">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
            <span class="sr-only">${cont.ln}</span>
          </a>
          <a href="${cont.linemail}" class="transition-colors escuro-icons claro-icons">
